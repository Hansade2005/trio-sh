import React from "react";
import { Card, CardHeader, CardTitle, CardContent } from "../ui/card";
import { Upload } from "lucide-react";
import { useShowExports } from "@/hooks/useShowExports";

export const DyadShowExports: React.FC<{ node?: any }> = ({ node }) => {
  const path = node?.properties?.path || "";
  const { data, isLoading, isError, error } = useShowExports({ path });

  return (
    <Card className="my-2">
      <CardHeader className="flex flex-row items-center gap-2 p-4 pb-2">
        <Upload className="text-blue-500" size={18} />
        <CardTitle className="text-base">Show Exports</CardTitle>
        <span className="ml-2 text-xs text-gray-500">{path}</span>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        {isLoading && <span className="text-amber-600 font-medium">Getting exports...</span>}
        {isError && <span className="text-red-600 font-medium">Error: {(error as any)?.message || data?.error}</span>}
        {data?.success && (
          <ul className="text-xs bg-gray-100 rounded p-2 mt-2">
            {data.exports.map((exp: string) => (
              <li key={exp}>{exp}</li>
            ))}
          </ul>
        )}
        {data && !data.success && data.error && <span className="text-red-600 font-medium">Error: {data.error}</span>}
      </CardContent>
    </Card>
  );
}; 