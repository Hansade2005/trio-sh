...
import { useShowImports } from "@/hooks/useShowImports";

export const DyadShowImports: React.FC<{ node?: any }> = ({ node }) => {
  const path = node?.properties?.path || "";
  const { data, isLoading, isError, error } = useShowImports({ path });

  return (
    <Card className="my-2">
      <CardHeader className="flex flex-row items-center gap-2 p-4 pb-2">
        <Download className="text-blue-500" size={18} />
        <CardTitle className="text-base">Show Imports</CardTitle>
        <span className="ml-2 text-xs text-gray-500">{path}</span>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        {isLoading && <span className="text-amber-600 font-medium">Getting imports...</span>}
